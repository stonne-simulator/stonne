...
void init_matrices() {
  ...
  prune<float>(KN_dense_matrix, KN_sparsity / 100.0f);

  std::size_t* p_MK_bitmap = generateBitMapFromDense(MK_dense_matrix.data(), M, K, GEN_BY_ROWS);
  std::size_t* p_KN_bitmap = generateBitMapFromDense(KN_dense_matrix.data(), K, N, GEN_BY_COLS);
  ...
  float* p_MK_sparse_matrix = generateMatrixSparseFromDense(MK_dense_matrix.data(), MK_bitmap.data(), M, K, GEN_BY_ROWS, MK_sparse_size);
  float* p_KN_sparse_matrix = generateMatrixSparseFromDense(KN_dense_matrix.data(), KN_bitmap.data(), K, N, GEN_BY_COLS, KN_sparse_size);
...
TEST_CASE("SmallSparseGEMM_SIGMA_Sim", "[sim][test]") {
  init_matrices();
  Stonne stonne = init();
  stonne.run();
...
TEST_CASE("SmallSparseGEMMe_Profiling", "[sim][benchmark]") {
  init_matrices();
