...
Stonne init() {
  Config stonne_cfg = {
      ...
      .m_SDMemoryCfg = {.mem_controller_type = OUTER_PRODUCT_GEMM, .n_read_ports = 16, .n_write_ports = 16},
  };
...
TEST_CASE("SmallOuterProductGEMM_Flexagon_Sim", "[sim][flexagon][test]") {
  init_matrices();
  Stonne stonne = init();
  stonne.run();
  ...
  REQUIRE(stonne.getNCycles() == 4541);
}

TEST_CASE("SmallOuterProductGEMM_Flexagon_Profiling", "[sim][flexagon][benchmark]") {
  init_matrices();
