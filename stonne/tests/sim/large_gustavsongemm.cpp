...
TEST_CASE("LargeGustavsonGEMM_Flexagon_Sim", "[sim][flexagon][test]") {
  init_matrices();
  Stonne stonne = init();
  stonne.run();
  ...
  REQUIRE(stonne.getNCycles() == 419055);
}

TEST_CASE("LargeGustavsonGEMM_Flexagon_Profiling", "[sim][flexagon][benchmark]") {
  init_matrices();
