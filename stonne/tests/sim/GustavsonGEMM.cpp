...
struct Test {
  ...
  Test(TestParams& p) : params(p) {
    ...
    denseToSparse(A_dense_matrix.data(), params.M, params.K, A_CSR_rowp, A_CSR_colidx, A_CSR_vals);
    ...
    params.stonneCfg.m_SDMemoryCfg.mem_controller_type = GUSTAVSONS_GEMM;
    params.stonneCfg.m_ASNetworkCfg.accumulation_buffer_enabled = false;

    // Memory configuration
    params.stonneCfg.m_SDMemoryCfg.input_address = 0;
    params.stonneCfg.m_SDMemoryCfg.weight_address = A_CSR_vals.size();
    params.stonneCfg.m_SDMemoryCfg.output_address = B_CSR_vals.size() + A_CSR_vals.size();
    ...
    auto main_memory = std::make_unique<SimpleMem<float>>(A_CSR_vals.size() + B_CSR_vals.size() + output.size());
    ...
    stonne->loadSparseOuterProduct("Test", params.N, params.K, params.M, A_CSR_vals.data(), B_CSR_vals.data(), A_CSR_colidx.data(), A_CSR_rowp.data(),
...
TEST_CASE("SmallGustavsonGEMM_Flexagon_Sim", "[sim][flexagon][test][small]") {
  Test t = smallTest();
  t.run();
  t.check();

  // Temporal check to ensure that I don't introduce errors during the refactor
  REQUIRE(t.stonne->getNCycles() == 4404);
}

TEST_CASE("LargeGustavsonGEMM_Flexagon_Sim", "[sim][flexagon][test][large]") {
  Test t = largeTest();
  t.run();
  t.check();

  // Temporal check to ensure that I don't introduce errors during the refactor
  REQUIRE(t.stonne->getNCycles() == 419055);
}

TEST_CASE("SmallGustavsonGEMM_Flexagon_Profiling", "[sim][flexagon][benchmark][small]") {
  BENCHMARK_ADVANCED("STONNE GustavsonGEMM Small Benchmark")(Catch::Benchmark::Chronometer meter) {
    Test t = smallTest();
...
TEST_CASE("LargeGustavsonGEMM_Flexagon_Profiling", "[sim][flexagon][benchmark][large]") {
  BENCHMARK_ADVANCED("STONNE GustavsonGEMM Large Benchmark")(Catch::Benchmark::Chronometer meter) {
    Test t = largeTest();
    meter.measure([&t] { t.run(); });
    return 0;
  };
