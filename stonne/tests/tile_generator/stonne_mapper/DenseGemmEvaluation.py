...
def evaluate(testname, num_ms, dn_bw, rn_bw, M, N, K, tolerance=DEFAULT_TOLERANCE, generator="StonneMapper"):
    ...
    for T_M in [2 ** i for i in range(0, int(ceil(log2(num_ms))) + 1)]:
        for T_N in [2 ** i for i in range(0, int(ceil(log2(num_ms))) + 1)]:
            for T_K in [2 ** i for i in range(0, int(ceil(log2(num_ms))) + 1)]:
                ...
                if (T_K == 1 and K > 1) or T_M * T_N * T_K != num_ms:
                    continue
            ...
            if T_N >= N * 2:
                break
        # if T_M has reached the largest value that makes sense for the mapping, cancel the loop
        if T_M >= M * 2:
            break
    ...
    EvaluationUtils.save_densegemm_results_csv(passed, testname, num_ms, dn_bw, rn_bw, M, N, K, generator,
                                               generatedtile,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--generator', type=str, choices=['mRNA', 'StonneMapper'], default='StonneMapper',
    ...
    evaluate('ManualEvaluation', args.num_ms, args.dn_bw, args.rn_bw, args.M, args.N, args.K, args.tolerance,
             args.generator)
