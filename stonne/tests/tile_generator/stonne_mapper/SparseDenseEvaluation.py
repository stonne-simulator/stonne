...
try:  # Try to import the local version first (usually works when executed from the command line with Python directly)
    ...
    import tests.tile_generator.stonne_mapper.EvaluationUtils as EvaluationUtils
...
def evaluate(testname, num_ms, dn_bw, rn_bw, M, N, K, sparsity, tolerance=DEFAULT_TOLERANCE, generator="StonneMapper"):
    print(
        f'### SparseDense evaluation of {generator} with M={M}, N={N}, K={K}, sparsity={sparsity} and tolerance={tolerance}')
    ...
    EvaluationUtils.save_sparsedense_results_csv(passed, testname, num_ms, dn_bw, rn_bw, M, N, K, sparsity, generator,
                                                 generatedtile,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--tolerance', type=float, default=DEFAULT_TOLERANCE,
                        help='Tolerance for deviation on the results.')
    parser.add_argument('--generator', type=str, choices=['StonneMapper'], default='StonneMapper',
                        help='Tile generator to use.')
    ...
    evaluate('ManualEvaluation', args.num_ms, args.dn_bw, args.rn_bw, args.M, args.N, args.K, args.sparsity,
             args.tolerance, args.generator)
