...
GustavsonsSpGEMMSDMemory::GustavsonsSpGEMMSDMemory(id_t id, std::string name, Config stonne_cfg, Connection* write_connection, Memory<float>& mem) : MemoryController(id, name), mem(mem) {
...
//	std::cout << "CONFIGURING_SORTING_PSUMS_DOWN" << std::endl;
	...
	waiting_idle_cycles++;
        if(((current_state == RECEIVING_SORT_TREE_UP) || (current_state == WAITING_FOR_NEXT_STA_ITER)) && this->sort_up_received_first_value && (mem.get_read_buffer_size() == 0) && (waiting_idle_cycles < 800)) {
