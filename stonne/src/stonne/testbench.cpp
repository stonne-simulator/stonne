        ...
        for (int ox = 0; ox < OX; ox++) {
          for (int oy = 0; oy < OY; oy++) {
            ...
            for (int c = 0; c < C; c++) {
              for (int r = 0; r < R; r++) {
                for (int s = 0; s < S; s++) {
                  outputs[n * output_size_n + ox * size_oy + oy * K * G + g * K + k] +=
                      input[n * input_size_n + ox * strides * size_y + oy * strides * C * G + r * size_y + s * C * G + g * C + c] *
...
bool run_single_test_cnn(DNNLayer* dnn_layer, std::size_t T_R, std::size_t T_S, std::size_t T_C, std::size_t T_K, std::size_t T_G, std::size_t T_N,
                         std::size_t T_X_, std::size_t T_Y_, Config stonne_cfg, std::size_t n_test) {
  float EPSILON = 0.001;
  std::size_t MAX_RANDOM = 100;  //Variable used to generate the random values
  //Extracting the parameters
  std::size_t R = dnn_layer->get_R();  // R
  std::size_t S = dnn_layer->get_S();  // S
  std::size_t C = dnn_layer->get_C();  // C
  std::size_t K = dnn_layer->get_K();  // K
  std::size_t G = dnn_layer->get_G();
  std::size_t N = dnn_layer->get_N();              // N
  std::size_t X = dnn_layer->get_X();              // X
  std::size_t Y = dnn_layer->get_Y();              // Y
  std::size_t strides = dnn_layer->get_strides();  // strides
  std::size_t X_ = dnn_layer->get_X_();            // X_
  std::size_t Y_ = dnn_layer->get_Y_();            // Y_
  //Creating arrays to store the ifmap ofmap and weights
  std::size_t ifmap_size = X * Y * C;
  std::size_t filter_size = R * S * (C / G) * K;
  std::size_t ofmap_size = X_ * Y_ * K;
  ...
  // TODO Random size just to test, remove later
  Memory<float> memHierarchy(100 * 1024 * 1024);                                                                          //Creating memory hierarchy
  Stonne* stonne_instance = new Stonne(stonne_cfg, memHierarchy);                                                         //Creating instance of the simulator
  ...
  for (int i = 0; i < ofmap_size; i++) {
    ...
    if (difference > EPSILON) {
      ...
      std::cout << "\033[1;31m[ERROR] \033[0m"
                << "Test " << n_test << " Failed: "
                << "HARDWARE_PARAMETERS=(num_ms=" << stonne_cfg.m_MSNetworkCfg.ms_size << ") LAYER=("
                << "R=" << R << ", S=" << S << ", C=" << C << ", K=" << K << ", N=" << N << ", X=" << X << ", Y=" << Y << ", strides=" << strides
                << ", X_=" << X_ << ", Y_=" << Y_ << ") TILE=("
                << "T_R=" << T_R << ", T_S=" << T_S << ", T_C=" << T_C << ", T_K=" << T_K << ", T_N=" << T_N << ", T_X_=" << T_X_ << ", T_Y_=" << T_Y_ << ")"
                << std::endl;  //by the moment we just print num_ms as hardware parameters
  ...
  std::cout << "\033[1;32m[OK] \033[0m"
            << "Test " << n_test << " passed: "
            << "HARDWARE_PARAMETERS=(num_ms=" << stonne_cfg.m_MSNetworkCfg.ms_size << ") LAYER=("
            << "R=" << R << ", S=" << S << ", C=" << C << ", K=" << K << ", N=" << N << ", X=" << X << ", Y=" << Y << ", strides=" << strides << ", X_=" << X_
            << ", Y_=" << Y_ << ") TILE=("
            << "T_R=" << T_R << ", T_S=" << T_S << ", T_C=" << T_C << ", T_K=" << T_K << ", T_N=" << T_N << ", T_X_=" << T_X_ << ", T_Y_=" << T_Y_ << ")"
            << std::endl;  //by the moment we just print num_ms as hardware parameters
...
void run_real_tests() {
  ...
  std::size_t T_R, T_S, T_C, T_K, T_G, T_N, T_X, T_Y;
...
void run_stonne_architecture_tests(layerTest layer, std::size_t num_ms) {
  std::size_t R, S, C, K, G, N, X, Y, strides;  //Layer parameters
  std::size_t T_R, T_S, T_C, T_K, T_G, T_N, T_X, T_Y;
  Config stonne_cfg;
...
void run_smart_tests() {
  ...
  std::size_t T_R = 3;   // T_R
  std::size_t T_S = 3;   // T_S
  std::size_t T_C = 1;   // T_C
  std::size_t T_K = 2;   // T_K
  std::size_t T_G = 2;   // T_G
