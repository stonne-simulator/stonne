...
AccumulationBuffer::AccumulationBuffer(stonne_id_t id, std::string name, Config stonne_cfg, std::size_t n_accumulators) : Unit(id, name) {
...
void AccumulationBuffer::configureSignals(Tile* current_tile, DNNLayer* dnn_layer, std::size_t ms_size, std::size_t n_folding) {

  this->NPSumsConfiguration(
      n_folding);  //All the accumulation buffers have the same folding iteration numbers which means that in this case all the VNs are similar
}
...
void AccumulationBuffer::NPSumsConfiguration(std::size_t n_psums) {
  for (std::map<int, Accumulator*>::iterator it = accumulatortable.begin(); it != accumulatortable.end(); ++it) {
...
void AccumulationBuffer::printConfiguration(std::ofstream& out, std::size_t indent) {}

//Printing stats
void AccumulationBuffer::printStats(std::ofstream& out, std::size_t indent) {
...
void AccumulationBuffer::printEnergy(std::ofstream& out, std::size_t indent) {
  /*
