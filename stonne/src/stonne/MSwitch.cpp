...
MSwitch::MSwitch(stonne_id_t id, std::string name, int num, Config stonne_cfg) : Unit(id, name) {
  this->num = num;
...
MSwitch::MSwitch(stonne_id_t id, std::string name, int num, Config stonne_cfg, Connection* outputConnection, Connection* inputConnection)
...
void MSwitch::setNWindows(std::size_t n_windows) {
...
void MSwitch::setNFolding(std::size_t n_folding) {
...
void MSwitch::setVirtualNeuron(std::size_t VN) {
  this->VN = VN;
...
#ifdef DEBUG_MSWITCH_FUNC
        std::cout << "[MSWITCH_FUNC] Cycle " << this->local_cycle << ", MSwitch " << this->num << " has received an IACTIVATION from the forwarding link"
                  << std::endl;
#endif
...
#ifdef DEBUG_MSWITCH_FUNC
        std::cout << "[MSWITCH_FUNC] Cycle " << this->local_cycle << ", MSwitch " << this->num << " has received an IACTIVATION from the distribution network"
                  << std::endl;
#endif
...
#ifdef DEBUG_MSWITCH_FUNC
        std::cout << "[MSWITCH_FUNC] Cycle " << this->local_cycle << ", MSwitch " << this->num << " has received a WEIGHT from the distribution network"
                  << std::endl;
#endif
        if (weight_fifo->size() ==
            n_folding) {  //If the fifo already has n_folding weights means that this weight is a new distirbution weight phase and therefore we clear the fifo
...
#ifdef DEBUG_MSWITCH_FUNC
        std::cout << "[MSWITCH_FUNC] Cycle " << this->local_cycle << ", MSwitch " << this->num << " has received a PSUM from the distribution network"
                  << std::endl;
#endif
...
void MSwitch::printConfiguration(std::ofstream& out, std::size_t indent) {
...
void MSwitch::printStats(std::ofstream& out, std::size_t indent) {
  out << ind(indent) << "{" << std::endl;  //TODO put ID
...
void MSwitch::printEnergy(std::ofstream& out, std::size_t indent) {
  /*
