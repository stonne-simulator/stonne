...
#include "SparseFlex_ASwitch.hpp"
#include <assert.h>
#include <math.h>
#include <iostream>
...
SparseFlex_ASwitch::SparseFlex_ASwitch(stonne_id_t id, std::string name, std::size_t level, std::size_t num_in_level, Config stonne_cfg,
                                       Connection* inputLeftConnection, Connection* inputRightConnection, Connection* forwardingConnection,
                                       Connection* outputConnection, Connection* memoryConnection)
    : SparseFlex_ASwitch(id, name, level, num_in_level, stonne_cfg) {  //Constructor
...
void SparseFlex_ASwitch::setMemoryConnection(Connection* memoryConnection, std::size_t busID, std::size_t inputID) {
  this->busID = busID;
...
#ifdef DEBUG_ASWITCH_FUNC
      std::cout << "[ASWITCH_FUNC] Cycle " << local_cycle << ", SparseFlex_ASwitch " << this->level << ":" << this->num_in_level << " has performed a 2:1 sum"
                << std::endl;
#endif
...
DataPackage* SparseFlex_ASwitch::perform_operation_3_operands(DataPackage* pck_left, DataPackage* pck_right, DataPackage* pck_forward) {
  if (pck_left->get_vn() != pck_right->get_vn()) {
    std::cout << "Ha petado en el SparseFlex_ASwitch: "
  ...
  switch (this->operation_mode) {
    case ADDER:  //SUM
#ifdef DEBUG_ASWITCH_FUNC
      std::cout << "[ASWITCH_FUNC] Cycle " << local_cycle << ", SparseFlex_ASwitch " << this->level << ":" << this->num_in_level << " has performed a 3:1 sum"
                << std::endl;
#endif
...
void SparseFlex_ASwitch::printConfiguration(std::ofstream& out, std::size_t indent) {
...
void SparseFlex_ASwitch::printStats(std::ofstream& out, std::size_t indent) {
  out << ind(indent) << "{" << std::endl;  //TODO put ID
...
void SparseFlex_ASwitch::printEnergy(std::ofstream& out, std::size_t indent) {
  /*
