...
class CompilerART : public CompilerComponent {
 private:
  ...
  //Aux functions
  void generate_art_signals(std::size_t num_ms);         //Generate the signals for the Adder swithces
  void generate_art_enabling_links(std::size_t num_ms);  //Generate the signals for the Adder switches

  //TODO. We can try to merge both sparse a non sparse. We do this now to keep the same interface
  //Aux sparse functions
  void generate_art_signals_sparse(std::size_t num_ms);         //Generate the signals for the Adder swithces in sparse mode (different VN sizes)
  void generate_art_enabling_links_sparse(std::size_t num_ms);  //Generate the signals for the Adder switches in sparse mode (different VN sizes)
 ...
 public:
  ...
  void configureSignals(Tile* current_tile, DNNLayer* dnn_layer, std::size_t num_ms, std::size_t n_folding);  //Print the stats of the component
