...
void CompilerMSN::configureSparseSignals(std::vector<SparseVN> sparseVNs, DNNLayer* dnn_layer, std::size_t num_ms) {
  assert(ispowerof2(num_ms));
...
void CompilerMSN::generate_ms_signals(std::size_t num_ms) {
  ...
  //std::size_t n_folding=(this->dnn_layer->get_R() / this->current_tile->get_T_R())*(this->dnn_layer->get_S() / this->current_tile->get_T_S()) * (this->dnn_layer->get_C() / this->current_tile->get_T_C());
  for (int i = 0; i < this->current_tile->get_Num_VNs(); i++) {
    for (int j = 0; j < this->current_tile->get_VN_Size(); j++) {
      std::size_t ms_index = i * this->current_tile->get_VN_Size() + j;
  ...
  if (this->current_tile->get_T_Y_() >
      1) {  //Conditions to enable the fw links. T_Y_ must be greater than 1 and stride must be 1. If not, all the signals must be false
    for (int i = 0; i < this->current_tile->get_Num_VNs(); i++) {
      for (int j = 0; j < this->current_tile->get_VN_Size(); j++) {
        std::size_t ms_index = i * this->current_tile->get_VN_Size() + j;
      ...
      for (int c = 0; c < this->current_tile->get_T_C(); c++) {    //For each channel
        for (int r = 0; r < this->current_tile->get_T_R(); r++) {  //For each row
          for (int s = 0; s < this->current_tile->get_T_S(); s++) {
            int ms_index = i * this->current_tile->get_VN_Size() + c * this->current_tile->get_T_R() * this->current_tile->get_T_S() +
                           r * this->current_tile->get_T_S() + s + shift_ms;  //Note that we sum shift_ms
...
//always disabled
void CompilerMSN::generate_ms_sparse_signals(std::size_t num_ms) {
  //1. Indicating to each MS its corresponding VN ID
  //Saving the number of iterations needed. Used in the cases where an extra MS is needed
  std::size_t ms_index = 0;
