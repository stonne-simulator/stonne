...
class SparseFlex_MSwitch : public Unit {
 private:
  ...
  std::size_t
      n_windows;  //Control number that specifies the number of slides (shifts) a MS takes to calculate one row. It is useful to know if the MS has to send/receive a data from the fw link in a specific cycle (the first cycle of a row cannot receive data from neighbours since there is no data of that row.
  std::size_t
      n_folding;  //Control the number of partial sums that must be generated to accumulate a whole ofmap value. if n_folding is 1, then partial sums is not required.

  //Counters to perform the control
  std::size_t current_n_windows;  //Measure the number of windows that have been performed in the current row.
  std::size_t
      current_n_folding;  // meausre the number of foldings that have been performed in the current window. This help to know whether to read from the input or from the fw link
  bool forward_psum;  //Indicates if the behaviour of this MS is to forward a psum. This is useful to implement folding
  bool
      direct_forward_psum;  //Always forward the psum. It is different than the variable forward_psum as the last one has some control regarding the number of iterations.
 ...
 public:
  ...
  void setVirtualNeuron(std::size_t VN);                          //Indicates the VN ID assigned
  ...
  void setNWindows(std::size_t n_windows);                        //The number of windows per row which is T_Y_
  void setNFolding(std::size_t n_folding);                        //The number of partial sums used to accumulate a whole sum
