#include "TileGenerator/mRNA/MrnaGenerator.hpp"
#include <cassert>
#include <iostream>
...
ConvTile MrnaGenerator::generateConvTile(std::size_t num_ms, std::size_t dn_bw, std::size_t rn_bw, std::size_t R, std::size_t S, std::size_t C, std::size_t K,
  ...
  if (analyzer.bestmap) {
    return ConvTile(analyzer.bestmap->kernel_x,   // T_X
...
DenseGemmTile MrnaGenerator::generateDenseGemmTile(std::size_t num_ms, std::size_t dn_bw, std::size_t rn_bw, std::size_t M, std::size_t N, std::size_t K,
                                                   Target target) {
  if (target == Target::NONE) {
    std::cerr << "Not mRNA target specified, aborting" << std::endl;
    assert(false);
...
DNNModel MrnaGenerator::createDNNModel(std::string layer_type, std::size_t R, std::size_t S, std::size_t C, std::size_t K, std::size_t G, std::size_t N,
...
DNNModel MrnaGenerator::createConvModel(std::size_t R, std::size_t S, std::size_t C, std::size_t K, std::size_t G, std::size_t N, std::size_t X, std::size_t Y,
...
DNNModel MrnaGenerator::createDenseGemmModel(std::size_t M, std::size_t N, std::size_t K) {
  return createDNNModel("FC", M, K, 1, 1, 1, N, K, 1, M, 1, 1);
}

// Generates a filename for the results generation of a CONV layer
std::string MrnaGenerator::getMrnaOutputFilename(std::size_t num_ms, std::size_t dn_bw, std::size_t rn_bw, std::size_t R, std::size_t S, std::size_t C,
                                                 std::size_t K, std::size_t G, std::size_t N, std::size_t X, std::size_t Y, std::size_t X_, std::size_t Y_,
                                                 std::size_t stride) {
  std::stringstream ss;
  ss << mrnaOutputBasename << "CONV"
     << "_num_ms" << num_ms << "_dn_bw" << dn_bw << "_rn_bw" << rn_bw << "_R" << R << "_S" << S << "_C" << C << "_K" << K << "_G" << G << "_N" << N << "_X" << X
     << "_Y" << Y << "_X_" << X_ << "_Y_" << Y_ << "_stride" << stride << ".txt";
  return ss.str();
}

// Generates a filename for the results generation of a DenseGEMM/FC layer
std::string MrnaGenerator::getMrnaOutputFilename(std::size_t num_ms, std::size_t dn_bw, std::size_t rn_bw, std::size_t M, std::size_t N, std::size_t K) {
  std::stringstream ss;
  ss << mrnaOutputBasename << "FC"
     << "_num_ms" << num_ms << "_dn_bw" << dn_bw << "_rn_bw" << rn_bw << "_M" << M << "_N" << N << "_K" << K << ".txt";
  return ss.str();
}
