#include "TileGenerator/mRNA/Analyzer.hpp"
#include <list>
...
void Analyzer::writeProfile(std::ofstream& profile, std::map<int, MappingStrategy*> mappings) {
  ...
  for (int i = 0; i < mapping_num; i++) {
    ...
    profile << "Tile Size: <"
            << "T_X = " << map->kernel_x << ", T_Y = " << map->kernel_y << ", T_C = " << map->kernel_c << ", T_K = " << map->kernel_n
            << ", T_N = " << map->kernel_in << ", T_X' = " << map->kernel_ox << ", T_Y' = " << map->kernel_oy << ">\n";
    ...
    for (int j = 0; j < confignum; j++) {
      ...
      if (j != confignum - 1) {
        ...
        for (std::vector<int>::iterator I = map->weight_unicast[j].begin(), E = map->weight_unicast[j].end(); I != E; I++) {
          int unicast = *I;
          profile << "(" << unicast << ") ";
        }
        profile << "\n";
        profile << "- Number of multicast in weights: ";
        for (std::vector<int>::iterator I = map->weight_multicast[j].begin(), E = map->weight_multicast[j].end(); I != E; I++) {
          int multicast = *I;
          profile << "(" << multicast << ") ";
        }
        profile << "\n";
        profile << "- Size of multicast in weights: ";
        for (std::vector<int>::iterator I = map->weightmulticast_size[j].begin(), E = map->weightmulticast_size[j].end(); I != E; I++) {
          int size = *I;
          profile << "(" << size << ") ";
        }
        profile << "\n";
        profile << "- Number of unicast in inputs: ";
        for (std::vector<int>::iterator I = map->input_unicast[j].begin(), E = map->input_unicast[j].end(); I != E; I++) {
          int unicast = *I;
          profile << "(" << unicast << ") ";
        }
        profile << "\n";
        profile << "- Number of multicast in inputs: ";
        for (std::vector<int>::iterator I = map->input_multicast[j].begin(), E = map->input_multicast[j].end(); I != E; I++) {
          int multicast = *I;
          profile << "(" << multicast << ") ";
        }
        profile << "\n";
        profile << "- Size of unicast in inputs: ";
        for (std::vector<int>::iterator I = map->inputmulticast_size[j].begin(), E = map->inputmulticast_size[j].end(); I != E; I++) {
          int size = *I;
          profile << "(" << size << ") ";
        }
        profile << "\n";
      ...
      if (show_energy) {
        ...
        profile << "- Normalized energy of Reduce Network: " << rn_access + reduceeng << " ( accessing RSs = " << rn_access
                << ", reduce operation = " << reduceeng << " )\n\n";
      } else {
        profile << "\n";
      }
      ...
      if (show_energy) {
        ...
        profile << "- Normalized energy of Multiplier Network: " << mslink + msreg + multi << " ( forward energy = " << mslink
                << ", accessing local buffer in MS = " << msreg << ", multiply operations = " << multi << " )\n\n";
      } else {
        profile << "\n";
      }
...
bool Analyzer::checkCNNInput() {
  bool correct = false;
  if (dnn_model->cnn_input->input_channel == dnn_model->cnn_filter->filter_channel &&
      dnn_model->cnn_output->output_channel == dnn_model->cnn_filter->filter_number) {
    correct = true;
  }
  return correct;
}
...
void Analyzer::CalculateDSN(MappingStrategy* map, int confignum, int type) {
  ...
  if (type == 0) {
    ...
    if (dnn_model->cnn_filter->window_stride == 1) {
      unsigned long long temp1 = ((unsigned long long)edge * (inputtrans - outputx * filtertrans) * (unsigned long long)(elementnum / filtery));
...
void Analyzer::CalculateMSN(MappingStrategy* map, int confignum, int type) {
  ...
  if (type == 0) {
    if (dnn_model->cnn_filter->window_stride == 1) {
      ...
      totalreg =
          elementnum * vnnum * outputx * filtertrans + vnnum * (elementnum / filtery) * (inputtrans - outputx * filtertrans) + elementnum * vnnum * inputtrans;
...
void Analyzer::CalculateSPM(MappingStrategy* map, int confignum, int type) {
  ...
  if (type == 0) {
    ...
    if (dnn_model->cnn_filter->window_stride == 1) {
      inputdata = (unsigned long long)(elementnum * outputx) * filtertrans + (unsigned long long)(elementnum / filtery) * (inputtrans - outputx * filtertrans);
    } else {
...
void Analyzer::CalculateDram(MappingStrategy* map, std::vector<double> partialsum, int type) {
  ...
  if (type == 0) {
    unsigned long long origndatatrans =
        (unsigned long long)(std::ceil(double((inputx * inputy * inputc + filterx * filtery * filterc * filtern + outputx * outputy * outputc) * 2) / 8));
    int pardata = 0;
    int lastpar = 0;
  ...
  //If the 2D filter size is small, but there must be reconfiguration, we using the weight stationary is better.
    ...
    for (std::vector<int>::iterator I = KernelChannel.begin(); I != KernelChannel.end(); I++) {
      ...
      if (vnpartialsum <= maeri->pe_size) {
        ...
        config2cycle =
            std::ceil(double(dnn_model->cnn_output->output_x * dnn_model->cnn_output->output_y * dnn_model->cnn_output->output_channel) / double(config2vnnum));
        ...
        map->setpeak_ur(config_num, peak_ur);
        config3cycle =
            std::ceil(double(dnn_model->cnn_output->output_x * dnn_model->cnn_output->output_y * dnn_model->cnn_output->output_channel) / double(config3vnnum));
...
bool Analyzer::checkRNNInput() {
  bool correct = false;
  if (dnn_model->cnn_input->input_x + dnn_model->dnn_hidden->hidden_x == dnn_model->cnn_filter->filter_y &&
      dnn_model->cnn_filter->filter_x == 4 * dnn_model->dnn_hidden->hidden_x) {
    correct = true;
  }
  return correct;
}
...
void Analyzer::AnalyzeRNN(std::ofstream& profile, OptGoal option) {
  ...
  CalculateEnergy(map, config_num);
  if (dnn_model->cnn_input->input_x + dnn_model->dnn_hidden->hidden_x <= maeri->pe_size) {
    total_ur = (config1_cycle * config1_ur + config3_cycle * config3_ur + config4_cycle * config4_ur) / double(config1_cycle + config3_cycle + config4_cycle);
  } else {
...
void Analyzer::AnalyzeCambricon(std::ofstream& profile) {
  ...
  if (filterx * filtery * filterc < rowsize) {
    ...
    std::size_t totalmul = part1_cycle * maeri->pe_size;
