#include "TileGenerator/StonneMapper/StonneMapperGenerator.hpp"
#include <algorithm>
#include <cmath>
#include <set>
#include "TileGenerator/StonneMapper/TileOptionDenseGemm.hpp"
...
DenseGemmTile StonneMapperGenerator::generateDenseGemmTile(std::size_t num_ms, std::size_t dn_bw, std::size_t rn_bw, std::size_t M, std::size_t N,
                                                           std::size_t K, Target target) {
...
SparseDenseTile StonneMapperGenerator::generateSparseDenseTile(std::size_t num_ms, std::size_t dn_bw, std::size_t rn_bw, std::size_t M, std::size_t N,
                                                               std::size_t K, float MK_sparsity, Target target) {
  ...
  /* Implementation currently discarded because we don't consider the heuristic option
        ...
        std::set<TileOptionSparseDense> options;
        for (std::size_t T_K = (K == 1 ? 1 : 2); T_K <= K; T_K++) {
            std::size_t T_N = std::min(N, num_ms / T_K);
  ...
  std::size_t maxPowerOfTwoT_K = log2(num_ms) - 1;
  ...
  std::size_t power = round(maxPowerOfTwoT_K * approximatePowerFactorT_K(MK_sparsity));
  // Take care with the last subtract! Consider the K==1 -> T_K=1 case
  if (K == 1)
    power = 0;
  else
    power++;  // adds again the value, minimizing the minimum possible power to 1 (T_K>=2^1)

  // Get T_K using the calculated power of 2
  std::size_t T_K = std::min(K, (std::size_t)pow(2, power));
