...
class SparseFlex_ASwitch : public Unit {
 private:
  std::size_t level;             // Level where the Adder is set in the tree
  std::size_t num_in_level;      // Number of the Adder in the level
  std::size_t num_ms;            // These three parameters are for routing. In hardware it is not neccesary since it is used a bit vector
  std::size_t input_ports;       // Input port per branch (left, right).
  std::size_t output_ports;      // output port
  std::size_t forwarding_ports;  //Intermedium links between some nodes of the same level
  std::size_t buffers_capacity;  //Buffers size in bytes
  std::size_t port_width;        //Bit width of each port

  std::size_t busID;                 //CollectionBus connected to this SparseFlex_ASwitch
  std::size_t inputID;               //Number of input of the Collection Bus busID connected to this AS.
  std::size_t accumulationBufferID;  //Number of the accumulation buffer if used
  ...
  Connection*
      forwardingConnection;  // connection from the neighbour Adder. In hardware we use two. One for inputs and another for outputs and the adder slect the correct one
 ...
 public:
  SparseFlex_ASwitch(stonne_id_t id, std::string name, std::size_t level, std::size_t num_in_level, Config stonne_cfg);
  SparseFlex_ASwitch(stonne_id_t id, std::string name, std::size_t level, std::size_t num_in_level, Config stonne_cfg, Connection* inputLeftConnection,
  ...
  void setMemoryConnection(Connection* memoryConnection, std::size_t busID,
                           std::size_t inputID);  // Set the memory connection of the adder. Indicates the line connected to be shown in the stats.
  void resetSignals();                            //Reset all configuration signals

  // Getters
  std::size_t getLevel() const { return this->level; }

  std::size_t getNumInLevel() const { return this->num_in_level; }

  std::size_t getInputPorts() const { return this->input_ports; }  // Get the input bw per branch

  std::size_t getOutputPorts() const { return this->output_ports; }  // Get the output ports

  std::size_t getForwardingPorts() const { return this->forwarding_ports; }  // Get the forwarding ports

  cycles_t getLatency() const { return this->latency; }  // Get the number of cycles to compute

  fl_t getFlDirection() const { return this->fl_direction; }  // Get the direction of the fl for the current configuration. RECEIVE or SEND

  bool isFwEnabled() const { return this->fw_enabled; }

  std::size_t getAccumulationBufferID() const { return this->accumulationBufferID; }

  adderconfig_t getConfig() const { return this->config_mode; }  // Get the current configuration of the adder. ADD_2_1, ADD_3_1, ADD_1_1_PLUS_FW_1_1 or  FW_2_2

  adderoperation_t getOperationMode() const { return this->operation_mode; }  // Get the current operation configured (ADD or POOL) TODO POOL NOT SUPPORTED YET
  ...
  void printConfiguration(
      std::ofstream& out,
      std::size_t indent);  //This function prints the configuration of SparseFlex_ASwitch such as  the operation mode, augmented link enabled, etc
