#ifndef POOLSDMEMORY_H
#define POOLSDMEMORY_H
...
public:
    ...
    VNATPool_Register(unsigned int VN, unsigned int addr, unsigned int N, unsigned int G, unsigned int K, unsigned int X, unsigned int Y,
...
class PoolSDMemory : public MemoryController {
private:
    ...
    VNATPool_Register** VNAT;  //VNAT with as many registers as VN configured in the accelerator
...
public:
    PoolSDMemory(id_t id, std::string name, Config stonne_cfg, Connection* write_connection);
    ~PoolSDMemory();
