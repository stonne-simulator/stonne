...
typedef std::size_t stonne_id_t;
typedef std::size_t cycles_t;
typedef float* address_t;
typedef std::size_t counter_t;
typedef std::size_t latency_t;
typedef std::size_t* metadata_address_t;
...
enum adderconfig_t {
  ...
  FOLD,
  SORT_TREE
};  // To the best of my knowledge, FW_2_2 corresponds with sum left and right and send the result to the FW.
...
enum SparsityControllerState {
  ...
  ALL_DATA_SENT,
  CONFIGURING_SORTING_PSUMS_DOWN,
  SENDING_SORT_TREE_DOWN,
  CONFIGURING_SORTING_PSUMS_UP,
  RECEIVING_SORT_TREE_UP
};
