...
class Stats {
 ...
 public:
  ...
  virtual void print(std::ofstream& out, std::size_t indent) = 0;
};
...
class ConnectionStats : public Stats {
 ...
 public:
  ...
  ConnectionStats();
  void reset();
  void print(std::ofstream& out, std::size_t indent);
};
...
class MSwitchStats : public Stats {
 ...
 public:
  ...
  counter_t
      n_psum_forwarding_send;  //N cycles the MSwitch forwards a psum (i.e., there is folding and therefore this MSwitch is forwarding the psums to the ART.
