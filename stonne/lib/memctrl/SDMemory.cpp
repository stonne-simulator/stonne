...
VNAT_Register::VNAT_Register(std::size_t VN, std::size_t addr, std::size_t N, std::size_t G, std::size_t K, std::size_t X, std::size_t Y, std::size_t iter_N,
                             std::size_t iter_G, std::size_t iter_K, std::size_t iter_X, std::size_t iter_Y, std::size_t iter_R, std::size_t iter_S,
                             std::size_t iter_C, DNNLayer* dnn_layer, Tile* current_tile) {
  this->VN = VN;
...
std::size_t VNAT_Register::get_address() {
...
void VNAT_Register::update() {
  ...
  this->addr = this->base_addr + (index_N)*dnn_layer->get_X_() * dnn_layer->get_Y_() * dnn_layer->get_K() * dnn_layer->get_G() * word_size +
               index_X * dnn_layer->get_Y_() * dnn_layer->get_K() * dnn_layer->get_G() * word_size +
               index_Y * dnn_layer->get_K() * dnn_layer->get_G() * word_size + index_G * dnn_layer->get_K() * word_size + index_K * word_size;
  //std::cout << "Address: " << this->addr << std::endl;
}

SDMemory::SDMemory(stonne_id_t id, std::string name, Config stonne_cfg, Connection* write_connection, Memory<float>& mem)
...
SDMemory::~SDMemory() {
  delete write_fifo;
  if (this->tile_loaded) {
    std::size_t num_vns = this->current_tile->get_Num_VNs();
...
void SDMemory::setTile(Tile* current_tile) {
  ...
  this->current_tile = current_tile;
  std::size_t num_vn = this->current_tile->get_Num_VNs();
...
void SDMemory::setLayer(DNNLayer* dnn_layer, address_t input_address, address_t filter_address, address_t output_address, Dataflow dataflow) {
  ...
  /*
                    std::size_t filter_size = dnn_layer->get_R()*dnn_layer->get_S()*dnn_layer->get_C(); //R*S*C
                    std::size_t input_size = dnn_layer->get_X()*dnn_layer->get_Y()*dnn_layer*get_C(); // ISize = X*Y*X
                    std::size_t row_input_size = dnn_layer->get_C()*dnn_layer->get_Y(); //Row input size = C*Y
                    std::size_t output_size = dnn_layer->get_X_()*dnn_layer->get_Y_()*dnn_layer->get_K(); //Osize = X'*Y'*K
                    std::size_t channel_output_size = dnn_layer->get_X_()*dnn_layer->get_Y_();  //output channel size= X'*Y'
                    std::size_t row_output_size = dnn_layer->get_K()*dnn_layer->get_Y_();
                    std::size_t stride = dnn_layer->get_stride();
                    ...
                    std::size_t current_T_R = current_tile->get_T_R();
                    std::size_t current_T_S = current_tile->get_T_S();
                    std::size_t current_T_C = current_tile->get_T_C();
                    std::size_t current_T_K = k;
                    std::size_t current_T_N = n;
                    std::size_t current_T_X_ = x_;
                    std::size_t current_T_Y_ = y_;
...
void SDMemory::cycle() {
  ...
  std::size_t current_iteration = current_output_pixel / output_psums_per_channel;
  unsigned index_G = current_G * this->current_tile->get_T_G();
  unsigned index_K = current_K * this->current_tile->get_T_K();
  std::size_t pck_iteration = (index_G) * this->dnn_layer->get_K() + index_K * this->current_tile->get_T_G();
  ...
  if ((mem.get_read_buffer_size() == 0) && (mem.get_write_buffer_size() < this->n_write_mshr)) {
    ...
    if (!this->input_finished && (pck_iteration <= current_iteration)) {  //If
      ...
      if (!this->weights_distributed) {
        ...
        if (current_tile->get_T_N() * current_tile->get_T_X_() * current_tile->get_T_Y_() >
            1) {  //If N*X_*Y_ is greater than 1 then MULTICAST MESSAGE as the same weight is sent multiple times.
                ...
                for (unsigned r = 0; r < current_tile->get_T_R(); r++) {
                  for (unsigned s = 0; s < current_tile->get_T_S(); s++) {
                    ...
                    for (int n = 0; n < this->current_tile->get_T_N(); n++) {
                      unsigned desp_n = n * this->current_tile->get_T_G() * this->current_tile->get_T_K() * this->current_tile->get_T_X_() *
                    ...
                    uint64_t new_addr = this->weight_dram_location +
                                        this->data_width * ((index_G + g) * this->group_size + (index_K + k) * this->filter_size +
                                                            (index_R + r) * this->row_filter_size + (index_S + s) * dnn_layer->get_C() + (index_C + c));
                    data_t data = 0.0;
                    // old_data = filter_address[(index_G+g)*this->group_size*word_size + (index_K+k)*this->filter_size*word_size + (index_R+r)*this->row_filter_size*word_size + (index_S+s)*dnn_layer->get_C()*word_size + (index_C+c)];  //Fetching. Note the distribution in memory is interleaving the channels
              ...
              for (unsigned c = 0; c < current_tile->get_T_C(); c++) {
                for (unsigned r = 0; r < current_tile->get_T_R(); r++) {
                  for (unsigned s = 0; s < current_tile->get_T_S(); s++) {
                    ...
                    unsigned index_S = current_S * this->current_tile->get_T_S();
                    this->sdmemoryStats.n_SRAM_weight_reads++;  //To track information
                    uint64_t new_address = this->weight_dram_location +
                                           this->data_width * ((index_G + g) * this->group_size + (index_K + k) * this->filter_size +
                                                               (index_R + r) * this->row_filter_size + (index_S + s) * dnn_layer->get_C() + (index_C + c));
                    data_t data = 0.0;

                    // old_data = filter_address[(index_G+g)*this->group_size*word_size + (index_K+k)*this->filter_size*word_size +
                    ...
                    std::size_t receiver = g * current_tile->get_T_K() * window_size + k * window_size + c * current_tile->get_T_R() * current_tile->get_T_S() +
                                           r * current_tile->get_T_S() + s + folding_shift;  //+1 because of the ms we leave free for the folding
        ...
        std::size_t y_init = 0;  //y_init is the first column of the window to send. This might be 0, if all the window must be sent,
        //If the fw links are enabled because the tile allows it
        if ((this->current_tile->get_T_Y_() == 1) && (this->current_tile->get_T_S() > 1) &&
            (this->dnn_layer->get_strides() == 1)) {  //Conditions in which the fw links of the MSwitches are enabled
        ...
        for (unsigned x = 0; x < x_inputs; x++) {
          for (unsigned y = y_init; y < y_inputs; y++) {  //the number of columns to iterate depends on if the fw links of the MS are enabled
            ...
            for (int i = first_possible_vn_x; i <= last_possible_vn_x; i++) {  //TODO check with strides-1
              ...
              if ((i >= 0) && ((first_element_vn_i + ((this->current_tile->get_T_R() - 1))) < x_inputs) && (x <= last_element_vn_i) &&
                  (x >= first_element_vn_i)) {  // If it's a valid vn //This one, not the next
                // Y loop (cols)
                ...
                for (int j = first_possible_vn_y; j <= last_possible_vn_y; j++) {
                  ...
                  if ((j >= 0) && ((first_element_vn_j + (this->current_tile->get_T_S() - 1)) < y_inputs) && (y <= last_element_vn_j) &&
                      (y >= first_element_vn_j)) {  //If it's a valid vn
                    ...
                    for (int n = 0; n < this->current_tile->get_T_N(); n++) {  //For every n (batch size)
                      ...
                      int desp_n = n * this->current_tile->get_T_K() * this->current_tile->get_T_X_() * this->current_tile->get_T_Y_() *
  ...
  if (!write_fifo->isEmpty()) {
    ...
    for (int i = 0; i < write_fifo->size(); i++) {
      ...
      std::cout << "[MEM_OUTPUT] Cycle " << local_cycle << ", STONNE generated a partial output from output port " << pck_received->getOutputPort()
                << " and VN " << pck_received->get_vn() << ". OutputCount ( " << current_output_pixel << "/" << this->output_pixels_to_compute << ")"
                << std::endl;
#endif
...
void SDMemory::sendPackageToInputFifos(DataPackage* pck) {
  ...
  // UNICAST PACKAGE
    ...
    DataPackage* pck_new = new DataPackage(pck->get_size_package(), pck->get_data(), pck->get_data_type(), input_port, UNICAST, local_dest, 0,
                                           0);  //size, data, type, source (port), UNICAST, dest_local
...
void SDMemory::printStats(std::ofstream& out, std::size_t indent) {
...
void SDMemory::printEnergy(std::ofstream& out, std::size_t indent) {
  /*
