...
#include "OuterLoopSpGEMMSDMemory.hpp"
#include <assert.h>
#include <math.h>
#include <iostream>
#include "common/utility.hpp"

OuterLoopSpGEMMSDMemory::OuterLoopSpGEMMSDMemory(stonne_id_t id, std::string name, Config stonne_cfg, Connection* write_connection, Memory<float>& mem)
...
void OuterLoopSpGEMMSDMemory::setSparseMatrixMetadata(metadata_address_t MK_metadata_id, metadata_address_t MK_metadata_pointer,
...
void OuterLoopSpGEMMSDMemory::cycle() {
  ...
  //Getting some stats
  if ((current_state == CONFIGURING) || (current_state == DIST_STA_MATRIX) || (current_state == DIST_STR_MATRIX) ||
...
void OuterLoopSpGEMMSDMemory::printStats(std::ofstream& out, std::size_t indent) {
...
void OuterLoopSpGEMMSDMemory::printEnergy(std::ofstream& out, std::size_t indent) {
  /*
