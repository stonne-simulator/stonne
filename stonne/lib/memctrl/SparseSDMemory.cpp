...
void SparseSDMemory::setLayer(DNNLayer* dnn_layer, address_t MK_address, address_t KN_address, address_t output_address, Dataflow dataflow) {
  ...
  this->sta_counters_table = new std::size_t[dim_sta * K];
  this->str_counters_table = new std::size_t[dim_str * K];
}
...
void SparseSDMemory::cycle() {
  ...
  if ((mem.get_read_buffer_size() == 0)) {
    ...
    if (current_state == CONFIGURING) {    //If the architecture has not been configured
      ...
      if (this->sta_current_j_metadata > 0) {  // We are managing one cluster with folding
        ...
        if (this->configurationVNs.size() == 0 && n_ms > 0) {  //If any entire cluster fits (at least 1 element), then folding is needed to manage this cluster
          /*
                 ...
                 */
          SparseVN VN(n_current_cluster,
                      false);  //Here folding is still disabled as this is the first iteration
          this->configurationVNs.push_back(VN);
          this->vnat_table.push_back(0);  //Adding the current calculation (row or column) of this VN.
          //Searching if there are more values in the next cluster. If not, update sta_last_j_metadata to K to indicate that in the next iteration the next sta dim must be evaluated
          int remaining_values = 0;
      ...
      if (this->sta_current_j_metadata > 0) {  //If folding is enabled there is just a row on  fly
        ...
        //send psum
        std::size_t addr_offset = (sta_current_index_metadata)*OUT_DIST_VN + str_current_index * OUT_DIST_VN_ITERATION;
      ...
      for (int j = init_point_str; j < end_point_str; j++) {  //For each element in the current vector in the str matrix
        ...
        std::size_t first_sta_vector = sta_current_index_metadata;
        std::size_t last_sta_vector = sta_current_index_metadata + this->configurationVNs.size();
        for (int i = first_sta_vector; i < last_sta_vector; i++) {      //For each vector in the sta matrix could exist a value to send the str value
          if (STA_metadata[i * STA_DIST_VECTOR + j * STA_DIST_ELEM]) {  //ADD Destination
            std::size_t dest = sta_counters_table[i * STA_DIST_VECTOR + j * STA_DIST_ELEM];
        ...
        if (STR_metadata[str_current_index * STR_DIST_VECTOR + j * STR_DIST_ELEM]) {
          std::size_t src = str_counters_table[str_current_index * STR_DIST_VECTOR + j * STR_DIST_ELEM];
  ...
  if (!write_fifo->isEmpty()) {
    ...
    for (int i = 0; i < write_fifo->size(); i++) {
      ...
      std::size_t addr_offset = (sta_current_index_metadata + vn) * OUT_DIST_VN + vnat_table[vn] * OUT_DIST_VN_ITERATION;
  ...
  if ((current_state == CONFIGURING) && ((mem.get_read_buffer_size() == 0))) {
    ...
    if (sta_current_index_metadata >= this->dim_sta) {
      ...
      std::size_t sta_metadata_size = this->dim_sta * K;
      std::size_t str_metadata_size = this->dim_str * K;
      std::size_t sta_zeros = sta_metadata_size - this->n_ones_sta_matrix;
      std::size_t str_zeros = str_metadata_size - this->n_ones_str_matrix;
...
void SparseSDMemory::printStats(std::ofstream& out, std::size_t indent) {
...
void SparseSDMemory::printEnergy(std::ofstream& out, std::size_t indent) {
  /*
