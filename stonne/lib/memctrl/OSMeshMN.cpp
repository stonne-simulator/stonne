...
OSMeshMN::OSMeshMN(stonne_id_t id, std::string name, Config stonne_cfg) : MultiplierNetwork(id, name) {
  ...
  for (int i = 0; i < this->ms_rows; i++) {    //From root to leaves (without the MSs)
    for (int j = 0; j < this->ms_cols; j++) {  // From left to right of the structure
      ...
      std::size_t ms_id = i * this->ms_cols + j;
...
void OSMeshMN::configureSignals(Tile* current_tile, DNNLayer* dnn_layer, std::size_t ms_rows, std::size_t ms_cols) {
  ...
  std::map<std::pair<int, int>, std::size_t> ms_vn_configuration = compiler->get_ms_vn_configuration();
  ...
  for (int i = 0; i < ms_rows; i++) {
    for (int j = 0; j < ms_cols; j++) {
      ...
      std::size_t VN = ms_vn_configuration[ms_index];
...
void OSMeshMN::configureSparseSignals(std::vector<SparseVN> sparseVNs, DNNLayer* dnn_layer, std::size_t ms_size) {
...
void OSMeshMN::printConfiguration(std::ofstream& out, std::size_t indent) {
...
void OSMeshMN::printStats(std::ofstream& out, std::size_t indent) {
...
void OSMeshMN::printEnergy(std::ofstream& out, std::size_t indent) {
  /*
