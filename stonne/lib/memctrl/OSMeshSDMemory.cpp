...
OSMeshSDMemory::OSMeshSDMemory(stonne_id_t id, std::string name, Config stonne_cfg, Connection* write_connection)
...
void OSMeshSDMemory::cycle() {
  ...
  if (current_state == OS_CONFIGURING) {  //Initialize these for the first time
    this->sdmemoryStats.n_reconfigurations++;
    std::size_t remaining_M = M - (current_M * T_M);
    std::size_t remaining_N = N - (current_N * T_N);
  ...
  if (current_state == OS_DIST_INPUTS) {
    ...
    for (int i = 0; i < this->cols_used; i++) {
      ...
      DataPackage* pck_to_send = new DataPackage(sizeof(data_t), data, WEIGHT, 0, UNICAST, i, 0, 0);
    ...
    for (int i = 0; i < rows_used; i++) {
      ...
      DataPackage* pck_to_send = new DataPackage(sizeof(data_t), data, IACTIVATION, 0, UNICAST, i + this->ms_cols, 0, 0);
      this->sendPackageToInputFifos(pck_to_send);
    }
  ...
  if (!write_fifo.isEmpty()) {
    ...
    for (int i = 0; i < write_fifo.size(); i++) {
      ...
      std::size_t current_tile_M_pointer = (n_iterations_completed / this->iter_N) * this->T_M;  //Change this to change the dataflow
      std::size_t current_tile_N_pointer = (n_iterations_completed % this->iter_N) * T_N;
      std::size_t vn_M_pointer = vn / this->cols_used;
      std::size_t vn_N_pointer = vn % this->cols_used;
      std::size_t addr_offset = (current_tile_M_pointer + vn_M_pointer) * this->N + current_tile_N_pointer + vn_N_pointer;
      vnat_table[vn]++;
...
void OSMeshSDMemory::sendPackageToInputFifos(DataPackage* pck) {
  ...
  if (pck->isBroadcast()) {
    ...
    for (int i = 0; i < this->n_read_ports; i++) {
      ...
      DataPackage* pck_new = new DataPackage(pck->get_size_package(), pck->get_data(), pck->get_data_type(), i, BROADCAST, 0,
                                             0);  //Size, data, data_type, source (port in this case), BROADCAST
...
void OSMeshSDMemory::printStats(std::ofstream& out, std::size_t indent) {
...
void OSMeshSDMemory::printEnergy(std::ofstream& out, std::size_t indent) {
  /*
