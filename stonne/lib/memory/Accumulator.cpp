...
void Accumulator::send() {
  if (!output_fifo.isEmpty()) {
    ...
    std::cout << "[ACCUMULATOR_FUNC] Cycle " << local_cycle << ", Accumulator " << this->n_accumulator << " has sent a psum to memory (FORWARDING DATA)"
              << std::endl;
#endif
...
void Accumulator::printStats(std::ofstream& out, std::size_t indent) {
  out << ind(indent) << "{" << std::endl;  //TODO put ID
...
void Accumulator::printEnergy(std::ofstream& out, std::size_t indent) {
  /*
