...
std::set<std::size_t> getAllDivisors(std::size_t n) {
  std::set<std::size_t> divisors;
  for (std::size_t i = 1; i <= sqrt(n) + 1; i++) {
    if (n % i == 0) {
      divisors.insert(i);
      divisors.insert(n / i);
    }
...
void addTilesUsingDivisors(std::set<TileOptionDenseGemm>& options, std::size_t M, std::size_t N, std::size_t K, std::size_t num_ms, float minimumMsUtilization,
  ...
  std::set<std::size_t> mDivisors = getAllDivisors(M);
  mDivisors.merge(getAllDivisors(M - 1));
  std::set<std::size_t> nDivisors = getAllDivisors(N);
  nDivisors.merge(getAllDivisors(N - 1));
  std::set<std::size_t> kDivisors = getAllDivisors(K);
...
std::string getStonneMapperOutputFilename(std::string layerType, std::size_t num_ms, std::size_t dn_bw, std::size_t rn_bw, std::size_t M, std::size_t N,
                                          std::size_t K) {
  std::stringstream ss;
  ss << stonneMapperOutputBasename << layerType << "_num_ms" << num_ms << "_dn_bw" << dn_bw << "_rn_bw" << rn_bw << "_M" << M << "_N" << N << "_K" << K
     << ".txt";
  return ss.str();
}
