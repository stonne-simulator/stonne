...
void Stonne::loadDNNLayer(Layer_t layer_type, std::string layer_name, std::size_t R, std::size_t S, std::size_t C, std::size_t K, std::size_t G, std::size_t N,
...
void Stonne::loadCONVLayer(std::string layer_name, std::size_t R, std::size_t S, std::size_t C, std::size_t K, std::size_t G, std::size_t N, std::size_t X,
...
void Stonne::loadFCLayer(std::string layer_name, std::size_t N, std::size_t S, std::size_t K, address_t filter_address, address_t input_address,
...
void Stonne::loadGEMM(std::string layer_name, std::size_t N, std::size_t K, std::size_t M, address_t MK_matrix, address_t KN_matrix,
                      metadata_address_t MK_metadata, metadata_address_t KN_metadata, address_t output_matrix, metadata_address_t output_metadata,
                      Dataflow dataflow) {
...
void Stonne::loadDenseGEMM(std::string layer_name, std::size_t N, std::size_t K, std::size_t M, address_t MK_matrix, address_t KN_matrix,
                           address_t output_matrix, Dataflow dataflow) {
  //Setting GEMM (from SIGMA) parameters onto CNN parameters:
  //N=N
  //S and X in CNN =K in SIGMA
  //K in CNN = M in SIGMA
...
void Stonne::loadSparseDense(std::string layer_name, std::size_t N, std::size_t K, std::size_t M, address_t MK_matrix, address_t KN_matrix,
                             metadata_address_t MK_metadata_id, metadata_address_t MK_metadata_pointer, address_t output_matrix, std::size_t T_N,
                             std::size_t T_K) {
  //Setting GEMM (from SIGMA) parameters onto CNN parameters:
  //K in CNN=N here
  //C in CNN =K here
...
void Stonne::loadSparseOuterProduct(std::string layer_name, std::size_t N, std::size_t K, std::size_t M, address_t MK_matrix, address_t KN_matrix,
...
void Stonne::loadGEMMTile(std::size_t T_N, std::size_t T_K, std::size_t T_M) {
...
void Stonne::loadTile(std::size_t T_R, std::size_t T_S, std::size_t T_C, std::size_t T_K, std::size_t T_G, std::size_t T_N, std::size_t T_X_,
                      std::size_t T_Y_) {
  ...
  std::cout << "Loading Tile: <T_R=" << T_R << ", T_S=" << T_S << ", T_C=" << T_C << ", T_K=" << T_K << ", T_G=" << T_G << ", T_N=" << T_N << ", T_X'=" << T_X_
            << ", T_Y'=" << T_Y_ << ">" << std::endl;
  ...
  //End check
  std::size_t n_folding = (this->dnn_layer->get_R() / T_R) * (this->dnn_layer->get_S() / T_S) * (this->dnn_layer->get_C() / T_C);
  bool folding_enabled =
      false;  //Condition to use extra multiplier. Note that if folding is enabled but some type of accumulation buffer is needed this is false as no fw ms is needed.
  if ((n_folding > 1) && (this->stonne_cfg.m_ASNetworkCfg.accumulation_buffer_enabled == 0) &&
      (this->stonne_cfg.m_ASNetworkCfg.reduce_network_type !=
       FENETWORK)) {  //If there is folding and the RN is not able to acumulate itself, we have to use an extra MS to accumulate
  ...
  if (this->stonne_cfg.m_MSNetworkCfg.multiplier_network_type != OS_MESH) {  //IN TPU the configuration is done in the mem controller
    this->asnet->configureSignals(this->current_tile, this->dnn_layer, this->ms_size,
                                  n_folding);  //Calling the ART to configure the signals with them previously generated
                                               //Getting MN signals
...
void Stonne::loadFCTile(std::size_t T_S, std::size_t T_N, std::size_t T_K) {
...
void Stonne::loadSparseDenseTile(std::size_t T_N, std::size_t T_K) {
...
void Stonne::generateTile(TileGenerator::Generator generator, TileGenerator::Target target, float MK_sparsity) {
  ...
  TileGenerator::TileGenerator tileGenerator(stonne_cfg.m_MSNetworkCfg.ms_size, stonne_cfg.m_SDMemoryCfg.n_read_ports, stonne_cfg.m_SDMemoryCfg.n_write_ports,
                                             generator);

  switch (dnn_layer->get_layer_type()) {
    case Layer_t::CONV: {  // Generates a tile using the Stonne CONV parameters
      std::size_t R = dnn_layer->get_R();
      std::size_t S = dnn_layer->get_S();
      std::size_t C = dnn_layer->get_C();
      std::size_t K = dnn_layer->get_K();
      std::size_t G = dnn_layer->get_G();
      std::size_t N = dnn_layer->get_N();
      std::size_t X = dnn_layer->get_X();
      std::size_t Y = dnn_layer->get_Y();
      std::size_t strides = dnn_layer->get_strides();
      std::size_t X_ = (X - R + strides) / strides;
      std::size_t Y_ = (Y - S + strides) / strides;
      ...
      std::cout << "Generated tile: <T_R=" << tile.T_R << ", T_S=" << tile.T_S << ", T_C=" << tile.T_C << ", T_K=" << tile.T_K << ", T_G=" << tile.T_G
                << ", T_N=" << tile.T_N << ", T_X'=" << tile.T_X_ << ", T_Y'=" << tile.T_Y_ << ">" << std::endl;
    ...
    case Layer_t::GEMM: {  // Generates a tile using the Stonne FC parameters
      ...
      std::size_t M = dnn_layer->get_X();
      std::size_t N = dnn_layer->get_K();
      std::size_t K = dnn_layer->get_S();
...
void Stonne::printStats() {
  ...
  std::ofstream out;
  std::size_t num_ms = this->stonne_cfg.m_MSNetworkCfg.ms_size;
  std::size_t dn_bw = this->stonne_cfg.m_SDMemoryCfg.n_read_ports;
  std::size_t rn_bw = this->stonne_cfg.m_SDMemoryCfg.n_write_ports;
  ...
  out.open(output_directory_str + "output_stats_layer_" + this->dnn_layer->get_name() + "_architecture_MSes_" + std::to_string(num_ms) + "_dnbw_" +
           std::to_string(dn_bw) + "_" + "rn_bw_" + std::to_string(rn_bw) + "timestamp_" + std::to_string((int)time(NULL)) +
           ".txt");  //TODO Modify name somehow
  std::size_t indent = IND_SIZE;
  out << "{" << std::endl;
...
void Stonne::printGlobalStats(std::ofstream& out, std::size_t indent) {
  //std::size_t n_mswitches_used=this->current_tile->get_VN_Size()*this->current_tile->get_Num_VNs();
...
void Stonne::testMemory(std::size_t num_ms) {
  for (int i = 0; i < 20; i++) {
    this->mem->cycle();
    this->dsnet->cycle();
    this->msnet->cycle();
...
void Stonne::testTile(std::size_t num_ms) {
  ...
  for (auto it = switches_configuration.begin(); it != switches_configuration.end(); ++it) {
    ...
    std::cout << "Switch " << std::get<0>(current_node) << ":" << std::get<1>(current_node) << " --> " << get_string_adder_configuration(it->second)
              << std::endl;
  }
}

void Stonne::testDSNetwork(std::size_t num_ms) {
  //BRoadcast test
  /*
