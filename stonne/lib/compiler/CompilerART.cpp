...
void CompilerART::configureSignals(Tile* current_tile, DNNLayer* dnn_layer, std::size_t num_ms, std::size_t n_folding) {
...
void CompilerART::configureSparseSignals(std::vector<SparseVN> sparseVNs, DNNLayer* dnn_layer, std::size_t num_ms) {
  assert(ispowerof2(num_ms));
...
void CompilerART::generate_art_enabling_links(std::size_t num_ms) {
...
void CompilerART::generate_art_signals(std::size_t num_ms) {
  int nlevels = log10(num_ms) / log10(2);  //All the levels without count the leaves (MSwitches)
...
void CompilerART::generate_art_enabling_links_sparse(std::size_t num_ms) {
  //Ultimo nivel recorrido aparte hasta el numero de ms
  // Los siguientes niveles son recorridos mirando abajo y mirando en la configuracion del nivel inferior. Si tiene hijo
  int nlevels = log10(num_ms) / log10(2);  //All the levels without count the leaves (MSwitches)
  int ms_used = 0;
...
void CompilerART::generate_art_signals_sparse(std::size_t num_ms) {
  int nlevels = log10(num_ms) / log10(2);  //All the levels without count the leaves (MSwitches)
  std::size_t shift_this_vn = 0;
  //Vector initialization
  bool* vector_bits = new bool[num_ms];
  bool* parent_bits = new bool[num_ms];  //Vector bits with the parent result.
  direction_t dir;
