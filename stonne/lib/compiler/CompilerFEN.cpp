...
void CompilerFEN::configureSignals(Tile* current_tile, DNNLayer* dnn_layer, std::size_t num_ms, std::size_t n_folding) {
...
void CompilerFEN::generate_fen_enabling_links(std::size_t num_ms) {
  ...
  for (int i = 0; i < num_adders; i++) {
    ...
    std::size_t ms_left = i * 2;
    std::size_t ms_right = ms_left + 1;
  ...
  for (int l = nlevels - 2; l >= 0; l--) {  //Last level was configured previously
    ...
    for (int i = 0; i < num_adders; i++) {
      ...
      std::size_t as_left = i * 2;
      std::size_t as_right = as_left + 1;
      bool left_enabled = false;
      bool right_enabled = false;
...
void CompilerFEN::generate_fen_signals(std::size_t num_ms) {
  int nlevels = log10(num_ms) / log10(2);  //All the levels without count the leaves (MSwitches)
  ...
  for (int i = 0; i < this->current_tile->get_Num_VNs(); i++) {  //For each neuron
    ...
    for (int l = nlevels - 1; l >= 0; l--) {  //For each level starting from leaves
          ...
          if ((as % 2)) {  //The configuration is done 2 by 2 (pair by pair) when it is the left node of a fw link
            ...
            if ((left_span == 0) || (right_span == 0)) {  //If it's 0 in one side the fw link is disabled.
              ...
              if (left_span > 0) {          //update higher level. It may go to the left or to the parent
                ...
                //    vector_bits[s]=false;
                // }
                std::cout << "  Left span is greater than 0 and right span is 0 so aswitch bit " << as << " is set as true and aswitch bit " << as + 1
                          << " as false" << std::endl;
              ...
              if (right_span > 0) {
                ...
                //    vector_bits[s]=false;
                // }
                std::cout << "  Right span is greater than 0 and left span is 0 so aswitch bit " << as + 1 << " is set as true and aswitch bit " << as
                          << " as false" << std::endl;
      ...
      std::size_t n_bits_enabled = 0;
      std::size_t last_bit_enabled;
      ...
      if (!fw_to_mem_set && (n_bits_enabled == 1)) {  //If there is just one enabled
        ...
        if (l == 0) {
          ...
          std::size_t fw_as_level = l - 1;
          std::size_t fw_num = last_bit_enabled / 2;
          ...
          std::size_t current_node = last_bit_enabled;
          ...
          if (level_node_to_reduce < 0) {        // We use the special node to send to memory
            ...
            std::size_t node_to_send = current_node / 2;
