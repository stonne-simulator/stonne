#include "SparseFlex_ASNetwork.hpp"
#include <assert.h>
#include <math.h>
...
SparseFlex_ASNetwork::SparseFlex_ASNetwork(stonne_id_t id, std::string name, Config stonne_cfg, Connection* outputConnection) : ReduceNetwork(id, name) {
    ...
    for (int j = 0; j < adders_this_level; j++) {  // From left to right of the structure
      ...
      if (i == 0) {  //The first node is connected to the outputConnection.
        ...
        Connection* correspondingOutputConnection =
            inputconnectiontable[sourcepair];  //output node i is the input of the node -i, already connection in the previous loop (code below)
...
void SparseFlex_ASNetwork::setMemoryConnections(std::vector<std::vector<Connection*>> memoryConnections) {
  ...
  for (int i = 0; i < double_switches.size(); i += 2) {  //2 by 2 since each 2 aswitches form a single double switch
    ...
    connectionID += 2;
    std::cout << "SparseFlex_ASwitch " << as_first->getLevel() << ":" << as_first->getNumInLevel() << " connected to BUS " << busID << " INPUT " << inputID
              << std::endl;
    std::cout << "SparseFlex_ASwitch " << as_second->getLevel() << ":" << as_second->getNumInLevel() << " connected to BUS " << busID << " INPUT "
              << inputID + 1 << std::endl;
  }

  for (int i = 0; i < single_switches.size(); i++) {
    std::size_t inputID_Base = (double_switches.size() / n_bus_lines) +
    ...
    std::size_t busID = (i + double_switches.size()) % n_bus_lines;
...
void SparseFlex_ASNetwork::configureSignals(Tile* current_tile, DNNLayer* dnn_layer, std::size_t ms_size, std::size_t n_folding) {
...
void SparseFlex_ASNetwork::configureSparseSignals(std::vector<SparseVN> sparseVNs, DNNLayer* dnn_layer, std::size_t ms_size) {
  CompilerART* compiler_art = new CompilerART();  //Creating the object
...
void SparseFlex_ASNetwork::printConfiguration(std::ofstream& out, std::size_t indent) {
...
//Printing stats
void SparseFlex_ASNetwork::printStats(std::ofstream& out, std::size_t indent) {
...
void SparseFlex_ASNetwork::printEnergy(std::ofstream& out, std::size_t indent) {
  /*
