...
TemporalRN::TemporalRN(stonne_id_t id, std::string name, Config stonne_cfg, Connection* outputConnection) : ReduceNetwork(id, name) {
...
void TemporalRN::setMemoryConnections(std::vector<std::vector<Connection*>> memoryConnections) {
  ...
  for (int i = 0; i < this->accumulation_buffer_size; i++) {
    std::size_t inputID = (i / n_bus_lines);
    std::size_t busID = i % n_bus_lines;
...
void TemporalRN::configureSignals(Tile* current_tile, DNNLayer* dnn_layer, std::size_t ms_size, std::size_t n_folding) {
  this->accumulationBuffer->configureSignals(current_tile, dnn_layer, ms_size, n_folding);
}

void TemporalRN::configureSparseSignals(std::vector<SparseVN> sparseVNs, DNNLayer* dnn_layer, std::size_t ms_size) {
  assert(false);
}
...
void TemporalRN::printConfiguration(std::ofstream& out, std::size_t indent) {
...
//Printing stats
void TemporalRN::printStats(std::ofstream& out, std::size_t indent) {
  out << ind(indent) << "\"ASNetworkStats\" : {" << std::endl;
  this->accumulationBuffer->printStats(out, indent + IND_SIZE);
  out << ind(indent) << "}";
}

void TemporalRN::printEnergy(std::ofstream& out, std::size_t indent) {
  /*
