...
ASwitch::ASwitch(stonne_id_t id, std::string name, std::size_t level, std::size_t num_in_level, Config stonne_cfg) : Unit(id, name) {
  this->level = level;
...
ASwitch::ASwitch(stonne_id_t id, std::string name, std::size_t level, std::size_t num_in_level, Config stonne_cfg, Connection* inputLeftConnection,
...
void ASwitch::setMemoryConnection(Connection* memoryConnection, std::size_t busID, std::size_t inputID) {
  this->busID = busID;
...
void ASwitch::send() {
  ...
  if (!output_fw_fifo->isEmpty()) {
    ...
    while (!output_fw_fifo->isEmpty()) {  //TODO control bw
      ...
      std::cout << "[ASWITCH_FUNC] Cycle " << local_cycle << ", ASwitch " << this->level << ":" << this->num_in_level
...
#ifdef DEBUG_ASWITCH_FUNC
      std::cout << "[ASWITCH_FUNC] Cycle " << local_cycle << ", ASwitch " << this->level << ":" << this->num_in_level << " has sent a psum to the parent"
                << std::endl;
#endif
...
#ifdef DEBUG_ASWITCH_FUNC
      std::cout << "[ASWITCH_FUNC] Cycle " << local_cycle << ", ASwitch " << this->level << ":" << this->num_in_level << " has performed a 2:1 sum"
                << std::endl;
#endif
...
DataPackage* ASwitch::perform_operation_3_operands(DataPackage* pck_left, DataPackage* pck_right, DataPackage* pck_forward) {
  if (pck_left->get_vn() != pck_right->get_vn()) {
    std::cout << "Ha petado en el ASwitch: "
              ...
              << "Left VN=" << pck_left->get_vn() << " Right VN=" << pck_right->get_vn() << std::endl;
  }
  ...
  switch (this->operation_mode) {
    case ADDER:  //SUM
#ifdef DEBUG_ASWITCH_FUNC
      std::cout << "[ASWITCH_FUNC] Cycle " << local_cycle << ", ASwitch " << this->level << ":" << this->num_in_level << " has performed a 3:1 sum"
                << std::endl;
#endif
...
void ASwitch::printConfiguration(std::ofstream& out, std::size_t indent) {
  out << ind(indent) << "{" << std::endl;  //TODO put ID
  out << ind(indent + IND_SIZE) << "\"Configuration\" : \"" << get_string_adder_configuration(this->config_mode) << "\""
      << "," << std::endl;
  out << ind(indent + IND_SIZE) << "\"Augmented_link_enabled\" : " << this->fw_enabled << "," << std::endl;
  out << ind(indent + IND_SIZE) << "\"Augmented_link_direction\" : \"" << get_string_fwlink_direction(this->fl_direction) << "\""
      << "," << std::endl;
...
void ASwitch::printStats(std::ofstream& out, std::size_t indent) {
  out << ind(indent) << "{" << std::endl;  //TODO put ID
...
void ASwitch::printEnergy(std::ofstream& out, std::size_t indent) {
  /*
