...
DSwitch::DSwitch(stonne_id_t id, std::string name, std::size_t level, std::size_t num_in_level, Config stonne_cfg, std::size_t ms_size)
    : Unit(id, name) {  //ms_size depends on the local ms_size in the subtree
  this->level = level;
...
DSwitch::DSwitch(stonne_id_t id, std::string name, std::size_t level, std::size_t num_in_level, Config stonne_cfg, std::size_t ms_size,
                 Connection* leftConnection, Connection* rightConnection, Connection* inputConnection)
    : DSwitch(id, name, level, num_in_level, stonne_cfg, ms_size) {
  this->current_capacity = 0;
  this->pending_data = false;
...
void DSwitch::route_packages() {  //TODO It is supposing you have enouth bandwidth in the connection
  ...
  for (int i = 0; i < this->data.size(); i++) {  //For every package in the connection
    ...
    if (pck->isBroadcast()) {  //If the package is broacast, no routing is needed.
      ...
      std::size_t n_ms_to_route = num_ms >> (this->level);                 // num_ms  that switch can route  (num_ms / 2^level). level starts in 0
      std::size_t first_ms_to_route = this->num_in_level * n_ms_to_route;  //Shift of that certain switch. num_in_level starts in 0.
      //std::size_t last_ms_to_route = first_ms_to_route + n_ms_ro_route - 1; //The index in the array is -1
      ...
      std::size_t n_ms_to_route = num_ms >> (this->level);                 // num_ms  that switch can route  (num_ms / 2^level). level starts in 0
      std::size_t first_ms_to_route = this->num_in_level * n_ms_to_route;  //Shift of that certain switch. num_in_level starts in 0.
      std::size_t last_ms_to_route = first_ms_to_route + n_ms_to_route;    //The index in the array is -1
...
void DSwitch::printStats(std::ofstream& out, std::size_t indent) {
...
void DSwitch::printEnergy(std::ofstream& out, std::size_t indent) {
