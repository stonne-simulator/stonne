...
std::map<int, Connection*> DSNetworkTop::getLastLevelConnections() {
  ...
  for (int i = 0; i < this->n_input_ports; i++) {                                                 //For each tree we add its lastlevelconnections
    ...
    std::size_t index_base = i * ms_size_per_port;                //Current connection respect to the first connection in the first tree
...
void DSNetworkTop::printStats(std::ofstream& out, std::size_t indent) {
...
void DSNetworkTop::printEnergy(std::ofstream& out, std::size_t indent) {
  /*
