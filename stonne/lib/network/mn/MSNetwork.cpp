...
MSNetwork::MSNetwork(stonne_id_t id, std::string name, Config stonne_cfg) : MultiplierNetwork(id, name) {
...
void MSNetwork::virtualNetworkConfig(std::map<std::size_t, std::size_t> vn_conf) {
...
void MSNetwork::fwLinksConfig(std::map<std::size_t, bool> ms_fwsend_enabled, std::map<std::size_t, bool> ms_fwreceive_enabled) {
...
void MSNetwork::forwardingPsumConfig(std::map<std::size_t, bool> forwarding_psum_enabled) {
...
void MSNetwork::directForwardingPsumConfig(std::map<std::size_t, bool> direct_forwarding_psum_enabled) {
...
void MSNetwork::nWindowsConfig(std::size_t n_windows) {
  for (int i = 0; i < this->ms_size; i++) {
    MSwitch* ms = this->mswitchtable[i];
...
void MSNetwork::configureSignals(Tile* current_tile, DNNLayer* dnn_layer, std::size_t ms_size, std::size_t n_folding) {
...
void MSNetwork::configureSparseSignals(std::vector<SparseVN> sparseVNs, DNNLayer* dnn_layer, std::size_t ms_size) {
  CompilerMSN* compiler_msn = new CompilerMSN();
...
void MSNetwork::nFoldingConfig(std::map<std::size_t, std::size_t> n_folding_configuration) {
...
void MSNetwork::printConfiguration(std::ofstream& out, std::size_t indent) {
...
void MSNetwork::printStats(std::ofstream& out, std::size_t indent) {
...
void MSNetwork::printEnergy(std::ofstream& out, std::size_t indent) {
  /*
