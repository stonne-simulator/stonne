...
SparseFlex_MSwitch::SparseFlex_MSwitch(stonne_id_t id, std::string name, int num, Config stonne_cfg, Connection* outputConnection, Connection* inputConnection)
    : SparseFlex_MSwitch(id, name, num, stonne_cfg) {
...
void SparseFlex_MSwitch::setNWindows(std::size_t n_windows) {
...
void SparseFlex_MSwitch::setNFolding(std::size_t n_folding) {
...
void SparseFlex_MSwitch::setVirtualNeuron(std::size_t VN) {
  this->VN = VN;
    ...
    if (connection->existPendingData()) {
      ...
      for (int i = 0; i < data_received.size(); i++) {
        ...
        std::cout << "[MSWITCH_FUNC] Cycle " << this->local_cycle << ", SparseFlex_MSwitch " << this->num
                  << " has received an IACTIVATION from the forwarding link" << std::endl;
#endif
...
#ifdef DEBUG_MSWITCH_FUNC
        std::cout << "[MSWITCH_FUNC] Cycle " << this->local_cycle << ", SparseFlex_MSwitch " << this->num
                  << " has received an IACTIVATION from the distribution network" << std::endl;
#endif
...
DataPackage* SparseFlex_MSwitch::perform_operation_2_operands(DataPackage* pck_left, DataPackage* pck_right) {
  ...
  DataPackage* result_pck = new DataPackage(sizeof(data_t), result, PSUM, pck_right->get_source(), this->VN, MULTIPLIER, pck_right->getRow(),
...
void SparseFlex_MSwitch::printConfiguration(std::ofstream& out, std::size_t indent) {
...
void SparseFlex_MSwitch::printStats(std::ofstream& out, std::size_t indent) {
  out << ind(indent) << "{" << std::endl;  //TODO put ID
...
void SparseFlex_MSwitch::printEnergy(std::ofstream& out, std::size_t indent) {
  /*
