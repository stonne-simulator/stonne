...
void simulateDenseConvForward(std::string layer_name, float* input, float* weight, float* output, int R, int S, int C, int K, int G, int N, int X, int Y,
                              int X_, int Y_, int strides, int pad_x, int pad_y, std::string path_to_tile, Config stonne_cfg) {
...
*/
  ...
  //Tile parameters
  std::size_t T_R = tile.get_T_R();
  std::size_t T_S = tile.get_T_S();
  std::size_t T_C = tile.get_T_C();
  std::size_t T_K = tile.get_T_K();
  std::size_t T_G = tile.get_T_G();
  std::size_t T_N = tile.get_T_N();
  std::size_t T_X_ = tile.get_T_X_();
  std::size_t T_Y_ = tile.get_T_Y_();

  //Executing the accelerator
  Stonne* stonne_instance = new Stonne(stonne_cfg);
  stonne_instance->loadDNNLayer(CONV, layer_name, R, S, C, K, G, N, X + 2 * pad_x, Y + 2 * pad_y, strides, (address_t)ifmap_to_send, (address_t)filters_to_send,
...
void simulateSparseGemmForward(std::string layer_name, float* KN_matrix_raw, float* MK_matrix_raw, float* output_raw, int N, int G, int gemm_M, int gemm_K,
                               int gemm_N, float sparsity_level, Config stonne_cfg, Dataflow dataflow) {
  ...
  for (int n = 0; n < N; n++) {
    ...
    for (int g = 0; g < G; g++) {  //Every group is calculated with a different GEMM
      ...
      std::size_t* acc_bitmap = new std::size_t[gemm_M_grouped * gemm_N_grouped];  //Currently is not generated by the accelerator

      //Generating bitmaps
      std::size_t* MK_bitmap = generateBitMapFromDense(MK_dense_matrix, gemm_M_grouped, gemm_K_grouped, GEN_BY_ROWS);
      std::size_t* KN_bitmap = generateBitMapFromDense(KN_dense_matrix, gemm_K_grouped, gemm_N_grouped, GEN_BY_COLS);
      ...
      stonne_instance->loadGEMM(layer_name_group, gemm_N_grouped, gemm_K_grouped, gemm_M_grouped, MK_sparse_matrix, KN_sparse_matrix, MK_bitmap, KN_bitmap,
                                acc_output, acc_bitmap, dataflow);  //Loading GEMM
...
void simulateSparseGemmWithBitmapsForward(std::string layer_name, float* KN_matrix_raw, float* MK_matrix_raw, float* output_raw, int N, int G, int gemm_M,
                                          int gemm_K, int gemm_N, std::size_t* MK_bitmap, std::size_t* KN_bitmap, Config stonne_cfg, Dataflow dataflow) {
  ...
  for (int n = 0; n < N; n++) {
    ...
    for (int g = 0; g < G; g++) {  //Every group is calculated with a different GEMM
      ...
      std::size_t* pointer_MK_bitmap = (std::size_t*)MK_bitmap + weight_offset_ * g;
      std::size_t* pointer_KN_bitmap = (std::size_t*)KN_bitmap + col_offset_ * g;
      ...
      stonne_instance->loadGEMM(layer_name_group, gemm_N_grouped, gemm_K_grouped, gemm_M_grouped, MK_sparse_matrix, KN_sparse_matrix, pointer_MK_bitmap,
...
void simulateDenseGemmForward(std::string layer_name, float* KN_matrix_raw, float* MK_matrix_raw, float* output_raw, int N, int G, int gemm_M, int gemm_K,
                              int gemm_N, std::string path_to_tile, Config stonne_cfg) {
  if (path_to_tile == "") {
    std::cout << "Tile file parameters must be specified" << std::endl;
    exit(1);
  }
  ...
  for (int n = 0; n < N; n++) {
    ...
    for (int g = 0; g < G; g++) {  //Every group is calculated with a different GEMM
      ...
      stonne_instance->loadFCLayer(layer_name_group, gemm_N_grouped, gemm_K_grouped, gemm_M_grouped, (address_t)pointer_MK_dense_matrix,
