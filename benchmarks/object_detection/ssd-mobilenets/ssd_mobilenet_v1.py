...
import sys
sys.path.insert(1, '../utils_obj_detection/')
from anchor_generator import create_ssd_anchors
from utils_obj_detection import SimulatedConv2d_tf
from utils_obj_detection import BatchNorm2d
from utils_obj_detection import BiasAdd
from utils_obj_detection import nms
from utils_obj_detection import decode_boxes
...
def conv_bn(inp, oup, stride):
    tile_file = getTileFileFromConvDimensions(tiles_path, inp, oup, 3, 1)
    return nn.Sequential(
        OrderedDict(
            [
                ("0", SimulatedConv2d_tf(inp, oup, 3, simulation_file, tile_file, sparsity_ratio, stride, padding="SAME", bias=False)),
...
def create_mobilenetv1_ssd(num_classes, sim_file='../../../simulation_files/sigma_128mses_64_bw.cfg', tiles='tiles/accumulation_buffer/128_mses/', sparsity=0.0, stats_path=''):
...
def get_tf_pretrained_mobilenet_ssd(weights_file, sim_file='../../../simulation_files/sigma_128mses_64_bw.cfg', tiles='tiles/accumulation_buffer/128_mses/', sparsity=0.0, stats_path=''):
    from convert_tf_weights import get_state_dict, read_tf_weights

    model = create_mobilenetv1_ssd(91, sim_file, tiles, sparsity, stats_path)
