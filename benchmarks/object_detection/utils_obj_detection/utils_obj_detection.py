...
class SimulatedConv2d_tf(nn.SimulatedConv2d):
    """
    Conv2d with the padding behavior from TF
    """

    def __init__(self, *args, **kwargs):
        super(SimulatedConv2d_tf, self).__init__(*args, **kwargs)
    ...
    def forward(self, input):
        ...
        if self.padding == "VALID":
            output = torch_stonne.simulated_conv_forward(self.__class__.__name__, input, self.weight, self.stride, 0, self.dilation, self.groups, self.path_to_arch_file, self.path_to_tile, self.sparsity_ratio)
        ...
        output = torch_stonne.simulated_conv_forward(self.__class__.__name__, input, self.weight, self.stride, (padding_rows // 2, padding_cols // 2), self.dilation, self.groups, self.path_to_arch_file, self.path_to_tile, self.sparsity_ratio)
        if(self.bias is not None):
            output+=self.bias.view(1,-1,1,1)
        return output
